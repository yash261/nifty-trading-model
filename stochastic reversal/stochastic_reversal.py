...
# Create a Strategy
class stochastic_reversal(bt.Strategy):
    """A simple mean reverting strategy, slow stochastic is less than 10/15, trix >0 and current green candle for buy
    and slow stochastic is greater than 85/90, trix <0 and current red candle for buy """
    params = {"sma": 9,"trix": 3, "stochastic": 3, "variable1": 15 , "variable2":10 ,
              "optim": False, "optim_variable": (9,3,3,15,10)}  # Used for optimization;
    def __init__(self):
        ...
        self.trix = dict()
        self.sma = dict()
        self.stochastic = dict()

        if self.params.optim:  # Use a tuple during optimization
            self.params.sma,self.params.trix,self.params.stochastic,self.params.variable1,\
            self.params.variable2 = self.params.optim_variable  # variables replaced by tuple's contents

        # Can use this for sanity checks on variables
        # if self.params.ema_fast > self.params.ema_slow:
        #        raise ValueError(
        #         "A strategy cannot have the fast moving average's window be " + \
        ...
        for d in self.getdatanames():
            ...
            self.sma[d] = btind.SimpleMovingAverage(self.getdatabyname(d),  # The symbol for the moving average
                                                       ...
                                                       plotname="SMA: " + d)
            self.stochastic[d] = btind.StochasticSlow(self.getdatabyname(d),
                                                          period_dslow=self.params.stochastic,
                                                          plotname="stochastic: " + d)
    ...
    def next(self):
        ...
        for d in self.getdatanames():  # Looping through all symbols
            ...
            if pos == 0:  # Are we out of the market?
                if self.trix[d][0] > 0  and self.stochastic[d][0] < self.params.variable1 and self.dataclose[d][0]>self.dataopen[d][0] :# A buy signal
                   self.buy(data=self.getdatabyname(d),histnotify=True)

                if self.trix[d][0] < 0 and self.stochastic[d][0] > 100-self.params.variable2 and self.dataopen[d][0]>self.dataclose[d][0] and\
                        self.dataopen[d][-1]>self.dataclose[d][-1]  :  # A sell signal
...
if __name__ == '__main__':
    ...
    cerebro.optstrategy(stochastic_reversal,optim=True, optim_variable=windows )
